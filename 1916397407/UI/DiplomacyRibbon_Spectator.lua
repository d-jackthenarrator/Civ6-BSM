...
function UpdateLeaders()
	...
	if b_teamer == true then
		...
		if bspec_game == true then
			m_spec_team = Players[Game:GetProperty("SPEC_ID_1")]:GetTeam()	
			...
			if bspec_loc == true then
				...
				table.insert(sortedPlayers, Players[Game:GetProperty("SPEC_ID_1")] )	
...
function FinishAddingLeader( playerID:number, uiLeader:table, kProps:table)	
	...
	if (Game:GetProperty("SPEC_NUM") ~= nil) then
		for k = 1, Game:GetProperty("SPEC_NUM") do
			if ( Game:GetProperty("SPEC_ID_"..k)~= nil) then
				if k == 1 then
					if playerID == Game:GetProperty("SPEC_ID_"..k) then
						bmasterspec = true
						bIsSpec = true
					end
					else
