...
function CityBanner:IsVisible()
	if Game.GetLocalPlayer() >= 0 then
		...
		if pLocalPlayerVis:IsVisible(locX, locY) or bspec == true then
			return true;
...
function CityBanner:UpdateGovernor(pCity:table)
	...
	for i, pGovernor in ipairs(governors) do
		...
		if playerID ~= localPlayerID and bspec == false then
		...
		if not isCityState or playerID == localPlayerID or bspec == true then
