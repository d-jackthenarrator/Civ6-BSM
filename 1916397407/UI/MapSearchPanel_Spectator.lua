...
function GetPlotSearchTerms(data)
	...
	if(data.ResourceType ~= nil) then
		...
		if bResourceVisible or bspec == true then
	...
	if pPlayerVis ~= nil and pPlayerVis:IsVisible(data.X, data.Y) then
		...
		if pUnitList ~= nil then
			...
			for _,pUnit in pairs(pUnitList) do
				if (pPlayerVis:IsUnitVisible(pUnit) or bspec == true) and not pUnit:IsDelayedDeath() then
...
function IncrementalSearch()
	...
	print("IncrementalSearch()",eObserverID,pPlayerVis,bspec)
	...
	for iPlot = m_nLastPlotSearched + 1, nPlots - 1 do
		...
		if pPlayerVis:IsRevealed(iPlot) or bpsec == true then
